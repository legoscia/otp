...
-export([pem_decode/1, pem_encode/1, 
	 ...
	 pkix_crl_verify/2,
	 pkix_crl_issuer/1,
	 openssl_crl_issuer_hash/1,
	 openssl_cert_issuer_hash/1,
	 openssl_name_hash/1
	]).
...
%%--------------------------------------------------------------------
-spec openssl_crl_issuer_hash(CRL::binary()| #'CertificateList'{}) ->
                                     string().

%% Description: Generates hash of CRL issuer name.
%% The hash is the same as generated by "openssl crl -hash".
%%--------------------------------------------------------------------
openssl_crl_issuer_hash(CRL) ->
    openssl_name_hash(public_key:pkix_crl_issuer(CRL)).

%%--------------------------------------------------------------------
-spec openssl_cert_issuer_hash(Cert::binary()| #'OTPCertificate'{}) ->
                                      string().

%% Description: Generates hash of certificate issuer name.
%% The hash is the same as generated by "openssl x509 -issuer_hash".
%%--------------------------------------------------------------------
openssl_cert_issuer_hash(Cert) ->
    {ok, {_Serial, Issuer}} = public_key:pkix_issuer_id(Cert, other),
    openssl_name_hash(Issuer).

%%--------------------------------------------------------------------
-spec openssl_name_hash({rdnSequence, [#'AttributeTypeAndValue'{}]}) ->
                               string().

%% Description: Generates OpenSSL-style hash of a name.
%%--------------------------------------------------------------------
openssl_name_hash({rdnSequence, _Attributes} = Name) ->
    HashThis = encode_name_for_openssl_hash(Name),
    <<A, B, C, D, _/binary>> = crypto:hash(sha, HashThis),
    HashValue = (D bsl 24) bor
        (C bsl 16) bor
        (B bsl 8) bor
        A,
    lists:flatten(io_lib:format("~8.16.0b", [HashValue])).
...
encode_name_for_openssl_hash(Name) ->
    Normalised = normalise_name(Name),
    encode_name(Normalised).

normalise_name({rdnSequence, Attributes}) ->
    {rdnSequence, lists:map(fun normalise_attribute/1, Attributes)}.

normalise_attribute([#'AttributeTypeAndValue'{
                        type = _Type,
                        value = Binary} = ATV]) when is_binary(Binary) ->
    {{TypeInt, Value}, <<>>} = asn1rt_nif:decode_ber_tlv(Binary),
    %% Convert Binary to UTF-8, if necessary.
    Converted =
        case TypeInt of
            12 ->
                %% UTF8String
    ...
    NewBinary = asn1rt_nif:encode_ber_tlv({12, NormalisedString}),
    [ATV#'AttributeTypeAndValue'{value = NewBinary}];
normalise_attribute([#'AttributeTypeAndValue'{
                        type = _Type,
...
encode_name({rdnSequence, Attributes}) ->
    EncodedAttributes = lists:map(fun encode_attribute/1, Attributes),    
    iolist_to_binary(EncodedAttributes).

encode_attribute([#'AttributeTypeAndValue'{type = Type, value = ValueBin}]) when is_binary(ValueBin)->
    {TypeBin, _TypeLen} = 'OTP-PUB-KEY':'enc_AttributeType'(Type, [<<6>>]),
    Concatenated = <<(iolist_to_binary(TypeBin))/binary, ValueBin/binary>>,
    %% Ignore the leading 1F byte.  It is used to indicate a type
    %% identifier higher than 30, but it's not needed here.
    <<16#1f, Encoded/binary>> = asn1rt_nif:encode_ber_tlv({48, Concatenated}),
    <<16#1f, Wrapped/binary>> = asn1rt_nif:encode_ber_tlv({49, Encoded}),
    Wrapped.
